...
public class Parser {
    ...
    public AfterParse parse(String input) throws ShittyInputException {
        ...
        case "fixed":
            String[] splitString = args.split("takes");
            if (splitString.length < 2 || parts[1].trim().isEmpty()) {
                throw new ShittyInputException("Format error. Accepted format:\n" +
                        "fixed [task] takes [duration]");
            }
            String fixedDurationDesc = splitString[0].trim();
            String duration = splitString[1].trim();
            return new AfterParse("fixed", fixedDurationDesc, duration);
